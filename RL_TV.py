...
def RL_TV(im_extVol, otf, inner_iter, TV_reg, Nz, Ny, Nx, cp_stream):
    ...
    with cp_stream:
        ...
        wI = cupy.reshape(cupy.maximum(J1,0), (Nz, Ny, Nx), order='C')
        ...
        for k in range(inner_iter):
            ...
            ReBlurred = cupy.real(fft.ifftn(otf*fft.fftn(cupy.reshape(Y, (Nz, Ny, Nx), order='C')))).astype(cupy.float32)
            ...
            Ratio = cupy.real(fft.ifftn(cupy.conj(otf)*fft.fftn(cupy.reshape(ImRatio, (Nz, Ny, Nx), order='C')))).astype(cupy.float32)
            Ratio = cupy.reshape(Ratio, (Ratio.size,), order='C')
...
def computeTV(Image, TV_reg, Nz, Ny, Nx, cp_stream):
    epsilon = cupy.finfo(cupy.float32).eps
    with cp_stream:
        gx = cupy.diff(cupy.reshape(Image, (Nz, Ny, Nx), order='C'), 1, 2)
        ...
        gy = cupy.diff(cupy.reshape(Image, (Nz, Ny, Nx), order='C'), 1, 1)
